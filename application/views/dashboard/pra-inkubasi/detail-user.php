	...
	<div class="text container-fluid px-5">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb" style="font-weight: 400; background-color: #E4E9F7;">
				...
				<li class="breadcrumb-item" class="text-decoration-none"  aria-current="page"><a href="<?=base_url('dashboard/detailPraInkubasi/').$dataKelas['kelas']['id_kelas_training'];?>" class="text-decoration-none" style="color: #5A47AB;"><?=$dataKelas['kelas']['nama_kelas'];?></a></li>
			</ol>
		</nav>

		<div class="mx-5 mb-5 pr-5">
			<div class="head-home mt-3" style="border: solid 2px; border-color: #FBD15B;">
				<h2 class="pl-3" style="font-weight: 500; color: #5A47AB;"><?= $dataKelas['kelas']['nama_kelas'];?> - <?=$tenant['nama_tenant'];?></h2>
			</div>
							...
							<div style="min-height: 200px;">
								<?php foreach($dataKelas['detail'] as $dt):?>
									<?php if($pt['id_pertemuan_kelas'] == $dt['id_pertemuan_kelas'] && $dt['jenis'] == 'dokumen'):?>
										<p style="color: black;"><a href="<?=base_url('assets/dokumen/kelas/dokumen/').$dt['file'];?>" download><i class="fas fa-fw fa-file-pdf mr-2"><?=$dt['deskripsi'];?></a> <span class="text-muted ml-1">22kb</span><span style="float:right;"><input class="checkSelesai form-check-input" type="checkbox" <?=$dt['progress'];?> disabled></span></p>
									...
									<?php elseif($pt['id_pertemuan_kelas'] == $dt['id_pertemuan_kelas'] && $dt['jenis'] == 'assignment'):?>
										...
										<p>
											<a href="<?=base_url('dashboard/tugasUserPraInkubasi/').$dt['id_detail_kelas'];?>/<?=$tenant['id_tenant'];?>" class="text-decoration-none" style="color: black;"><i class="fas fa-fw fa-clipboard-list mr-2"></i></i><?=$judul[0];?></a>
									...
									<?php elseif($pt['id_pertemuan_kelas'] == $dt['id_pertemuan_kelas'] && $dt['jenis'] == 'link'):?>
										...
										<p style="color: black;">
											<a href="<?=base_url('dashboard/videoPraInkubasi/').$dt['id_detail_kelas'];?>" class="text-decoration-none" style="color: black;"><i class="fas fa-fw fa-link mr-2"></i></i><?= $judulLink[0];?></a>
											<span style="float:right;"><input class="checkSelesai form-check-input" type="checkbox" <?=$dt['progress'];?> disabled></span>
										</p>
									...
									<?php elseif($pt['id_pertemuan_kelas'] == $dt['id_pertemuan_kelas'] && $dt['jenis'] == 'page'):?>
										...
										<p>
											<i class="far fa-fw fa-window-restore mr-2" style="color: black;"></i><a href="<?=base_url('dashboard/pagePraInkubasi/').$dt['id_detail_kelas'];?>" class="text-decoration-none" style="color: black;"><?=$judul[0]?></a>
											<span style="float:right;"><input class="checkSelesai form-check-input" type="checkbox" <?=$dt['progress'];?> disabled></span>
										</p>
									<?php endif;?>
								<?php endforeach;?>
